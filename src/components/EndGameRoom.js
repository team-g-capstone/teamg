...
export default class EndGameRoom extends Component{
  ...
  render(){
    const playerNameLists = this.state.playersName.map((name, idx)=>{
        return (
        <View style={styles.playersList}key={idx}><Text style={styles.listText}>{`${name}:  `} {this.state.scores[idx]}</Text></View>
        )
    })
    return (
      ...
      <TouchableOpacity style={styles.touchButton} onPress={()=>this.props.navigation.navigate('Menu',{
            screen:"MainMenuNav",
